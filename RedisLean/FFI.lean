...
def connect (host := "127.0.0.1") (port : UInt32 := 6379) : EIO Error Ctx := hiredis.connect host port

def free (ctx : Ctx) : EIO Error Unit := hiredis.free ctx

def ping (ctx : Ctx) (msg : ByteArray) : EIO Error Bool := hiredis.ping ctx msg

def auth (ctx : Ctx) (password : String) : EIO Error Bool := hiredis.auth ctx password

def hello (ctx : Ctx) (protocol_version : UInt64 := 3) : EIO Error ByteArray := hiredis.hello ctx protocol_version
...
def get (ctx : Ctx) (k : ByteArray) : EIO Error ByteArray := hiredis.get ctx k

def del (ctx : Ctx) (keys : List ByteArray) : EIO Error UInt64 := hiredis.del ctx keys

def existsKey (ctx : Ctx) (key : ByteArray) : EIO Error Bool := hiredis.existsKey ctx key

def typeKey (ctx : Ctx) (key : ByteArray) : EIO Error String := hiredis.typeKey ctx key

def incr (ctx : Ctx) (key : ByteArray) : EIO Error UInt64 := hiredis.incr ctx key

def incrBy (ctx : Ctx) (key : ByteArray) (increment : Int64) : EIO Error UInt64 := hiredis.incrby ctx key increment

def decr (ctx : Ctx) (key : ByteArray) : EIO Error UInt64 := hiredis.decr ctx key

def decrBy (ctx : Ctx) (key : ByteArray) (decrement : Int64) : EIO Error UInt64 := hiredis.decrby ctx key decrement

def sismember (ctx : Ctx) (key member : ByteArray) : EIO Error Bool := hiredis.sismember ctx key member
...
def publish (ctx : Ctx) (channel : String) (message : ByteArray) : EIO Error UInt64 := hiredis.publish ctx channel message

def subscribe (ctx : Ctx) (channel : String) : EIO Error Bool := hiredis.subscribe ctx channel

def command (ctx : Ctx) (command_str : String) : EIO Error ByteArray := hiredis.command ctx command_str

def ttl (ctx : Ctx) (key : ByteArray) : EIO Error UInt64 := hiredis.ttl ctx key

def pttl (ctx : Ctx) (key : ByteArray) : EIO Error UInt64 := hiredis.pttl ctx key

def keys (ctx : Ctx) (pattern : ByteArray) : EIO Error (List ByteArray) := hiredis.keys ctx pattern

def hset (ctx : Ctx) (key field value : ByteArray) : EIO Error UInt64 := hiredis.hset ctx key field value

def hget (ctx : Ctx) (key field : ByteArray) : EIO Error ByteArray := hiredis.hget ctx key field

def hgetall (ctx : Ctx) (key : ByteArray) : EIO Error (List ByteArray) := hiredis.hgetall ctx key

def hdel (ctx : Ctx) (key field : ByteArray) : EIO Error UInt64 := hiredis.hdel ctx key field

def hexists (ctx : Ctx) (key field : ByteArray) : EIO Error Bool := hiredis.hexists ctx key field

def hincrby (ctx : Ctx) (key field : ByteArray) (increment : Int64) : EIO Error UInt64 := hiredis.hincrby ctx key field increment

def hkeys (ctx : Ctx) (key : ByteArray) : EIO Error (List ByteArray) := hiredis.hkeys ctx key

def zadd (ctx : Ctx) (key : ByteArray) (score : Float) (member : ByteArray) : EIO Error UInt64 := hiredis.zadd ctx key score member

def zcard (ctx : Ctx) (key : ByteArray) : EIO Error UInt64 := hiredis.zcard ctx key

def zrange (ctx : Ctx) (key : ByteArray) (start stop : Int64) : EIO Error (List ByteArray) := hiredis.zrange ctx key start stop
...
def xlen (ctx : Ctx) (key : ByteArray) : EIO Error UInt64 := hiredis.xlen ctx key

def xdel (ctx : Ctx) (key : ByteArray) (entry_ids : List ByteArray) : EIO Error UInt64 := hiredis.xdel ctx key entry_ids

def xtrim (ctx : Ctx) (key strategy : ByteArray) (max_len : UInt64) : EIO Error UInt64 := hiredis.xtrim ctx key strategy max_len
