// lpop :: UInt64 -> ByteArray -> Option UInt64 -> EIO Error (List ByteArray)
// Pop elements from the head of a list
// count: if None, pop 1 element; if Some n, pop n elements
lean_obj_res l_hiredis_lpop(uint64_t ctx, b_lean_obj_arg key, b_lean_obj_arg count_opt, lean_obj_arg w) {
  ...
  if (lean_is_scalar(count_opt)) {
    ...
    const char* argv[3] = {"LPOP", k, count_str};
    size_t argvlen[3] = {4, k_len, strlen(count_str)};
    r = (redisReply*)redisCommandArgv(c, 3, argv, argvlen);
  }

  if (!r) {
    lean_object* error = mk_redis_null_reply_error("LPOP returned NULL");
  ...
  if (r->type == REDIS_REPLY_NIL) {
    ...
    char error_msg[256];
    snprintf(error_msg, sizeof(error_msg), "LPOP returned unexpected reply type %d", r->type);
    freeReplyObject(r);
