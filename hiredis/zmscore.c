// zmscore :: UInt64 -> ByteArray -> List ByteArray -> EIO Error (List (Option Float))
// Get the scores of multiple members in a sorted set
lean_obj_res l_hiredis_zmscore(uint64_t ctx, b_lean_obj_arg key, b_lean_obj_arg members, lean_obj_arg w) {
  ...
  // Build argv: ZMSCORE key member [member ...]
  ...
  if (!r) {
    lean_object* error = mk_redis_null_reply_error("ZMSCORE returned NULL");
  ...
  if (r->type == REDIS_REPLY_ARRAY) {
    ...
    for (int i = (int)r->elements - 1; i >= 0; i--) {
      ...
      lean_object* opt_score;
      if (element->type == REDIS_REPLY_NIL) {
        opt_score = lean_box(0); // none
      } else if (element->type == REDIS_REPLY_STRING && element->str) {
        double score = strtod(element->str, NULL);
        opt_score = lean_alloc_ctor(1, 1, 0);
        lean_ctor_set(opt_score, 0, lean_box_float(score));
      } else {
        opt_score = lean_box(0); // none for unexpected types
      }
      lean_object* list_node = lean_alloc_ctor(1, 2, 0);
      lean_ctor_set(list_node, 0, opt_score);
    ...
    char error_msg[256];
    snprintf(error_msg, sizeof(error_msg), "ZMSCORE returned unexpected reply type %d", r->type);
    freeReplyObject(r);
