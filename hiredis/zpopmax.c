// zpopmax :: UInt64 -> ByteArray -> Option UInt64 -> EIO Error (List ByteArray)
// Remove and return members with highest scores
lean_obj_res l_hiredis_zpopmax(uint64_t ctx, b_lean_obj_arg key, b_lean_obj_arg count_opt, lean_obj_arg w) {
  ...
  if (!r) {
    lean_object* error = mk_redis_null_reply_error("ZPOPMAX returned NULL");
  ...
  if (r->type == REDIS_REPLY_ARRAY) {
    ...
    char error_msg[256];
    snprintf(error_msg, sizeof(error_msg), "ZPOPMAX returned unexpected reply type %d", r->type);
    freeReplyObject(r);
