// hscan :: UInt64 -> ByteArray -> UInt64 -> Option ByteArray -> Option UInt64 -> EIO Error (UInt64 × List ByteArray)
// Incrementally iterate hash fields
lean_obj_res l_hiredis_hscan(uint64_t ctx, b_lean_obj_arg key, uint64_t cursor, b_lean_obj_arg pattern_opt, b_lean_obj_arg count_opt, lean_obj_arg w) {
  ...
  if (!r) {
    lean_object* error = mk_redis_null_reply_error("HSCAN returned NULL");
  ...
  if (r->type == REDIS_REPLY_ARRAY && r->elements >= 2) {
    ...
    if (items_reply->type == REDIS_REPLY_ARRAY) {
      // Returns alternating field-value pairs as flat list
    ...
    char error_msg[256];
    snprintf(error_msg, sizeof(error_msg), "HSCAN returned unexpected reply type %d", r->type);
    freeReplyObject(r);
