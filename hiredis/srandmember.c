// srandmember :: UInt64 -> ByteArray -> Option Int64 -> EIO Error (List ByteArray)
// Get random members from a set without removing them
lean_obj_res l_hiredis_srandmember(uint64_t ctx, b_lean_obj_arg key, b_lean_obj_arg count_opt, lean_obj_arg w) {
  ...
  if (lean_is_scalar(count_opt)) {
    const char* argv[2] = {"SRANDMEMBER", k};
    size_t argvlen[2] = {11, k_len};
    ...
    const char* argv[3] = {"SRANDMEMBER", k, count_str};
    size_t argvlen[3] = {11, k_len, strlen(count_str)};
    r = (redisReply*)redisCommandArgv(c, 3, argv, argvlen);
  }

  if (!r) {
    lean_object* error = mk_redis_null_reply_error("SRANDMEMBER returned NULL");
  ...
  if (r->type == REDIS_REPLY_NIL) {
    ...
    char error_msg[256];
    snprintf(error_msg, sizeof(error_msg), "SRANDMEMBER returned unexpected reply type %d", r->type);
    freeReplyObject(r);
